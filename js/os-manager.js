...
class OSTecManager {
    ...
    setupEventListeners() {
        ...
        document.querySelectorAll('.menu-item').forEach(item => {
            item.addEventListener('click', () => {
                ...
                } else if (tool === 'align-icons') {
                    this.alignDesktopIcons(true);
                }
    ...
    normalizeImportedData(rawContent) {
        ...
        try {
            ...
            try {
                parsed = JSON.parse(atob(rawContent.trim()));
            } catch (secondError) {
                throw new Error('Arquivo inválido. Use um backup .txt/.json exportado pelo sistema.');
            }
        }
    ...
    setupDesktopIcons() {
        ...
        const savedLayout = this.loadDesktopIconLayout();
        icons.forEach((icon, index) => {
            const iconKey = icon.dataset.module || icon.dataset.tool || `icon-${index}`;
            const savedPosition = savedLayout[iconKey];
            if (savedPosition) {
    ...
    alignDesktopIcons(showFeedback = false) {
        document.querySelectorAll('.desktop-icon').forEach((icon) => this.snapIconToGrid(icon));
        this.saveDesktopIconLayout();
        if (showFeedback) {
            this.showNotification('Ícones alinhados com sucesso!', 'success');
        }
    }
    ...
    saveDesktopIconLayout() {
        ...
        document.querySelectorAll('.desktop-icon').forEach((icon, index) => {
            const iconKey = icon.dataset.module || icon.dataset.tool || `icon-${index}`;
            layout[iconKey] = {
                left: parseFloat(icon.style.left || '20'),
                top: parseFloat(icon.style.top || '20')
            };
        });
        localStorage.setItem('adra-tec-icon-layout', JSON.stringify(layout));
    }

    loadDesktopIconLayout() {
        try {
            return JSON.parse(localStorage.getItem('adra-tec-icon-layout') || '{}');
        } catch (error) {
            return {};
        }
    }
